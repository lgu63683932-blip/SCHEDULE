...
export const ApprovalRequestForm: React.FC = () => {
  ...
  const existing = id ? getDocument(id) as ApprovalRequestDoc | undefined : undefined
  ...
  const handleSave = (submit = false) => {
    ...
    if (existing && id) {
      ...
      if (submit) {
        setTimeout(() => { navigate(`/approval/document/${newId}`) }, 0)
      } else {
        ...
        <div className="bg-white rounded-2xl border border-gray-200 overflow-hidden">
          <div className="px-8 py-5 border-b border-gray-100 bg-gray-50">
            <div className="flex items-center gap-2 text-sm text-gray-500 mb-1">
              <span>📋</span><span>품의서</span>
            </div>
            <h1 className="text-xl font-bold text-gray-900">{existing ? '품의서 수정' : '품의서 작성'}</h1>
          </div>
          ...
          <div className="px-8 py-6 space-y-5">
            ...
            {[
              ...
              { label: '세부내용 *', value: details, onChange: setDetails, placeholder: '세부 내용을 입력하세요', rows: 6 },
